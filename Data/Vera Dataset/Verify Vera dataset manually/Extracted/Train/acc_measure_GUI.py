...
### Utility functions
def renameSave(filename,scanList,entryList,metaList,
	header=['filename','ID']):
	with open(filename,'w') as writeFile:
		writer = csv.writer(writeFile, lineterminator='\n')
		writer.writerow(header)
		if len(header)<3:
			writer.writerows([[scn,ent] for scn,ent in 
				zip(scanList,entryList)])
		else:
			writer.writerows([[scn,ent]+meta for scn,ent,meta in 
				zip(scanList,entryList,metaList)])
...
container = tk.LabelFrame(root, 
	text='Bengali.AI Common Graphemes in Context')
...
studentid = tk.Entry(container, text='Student ID/Roll', 
	bd =5, width=50, textvariable=dispText)
...
def rightKey(e):
	...
	if idx>len(scanList)-1:
		idx=len(scanList)-1
	img = ImageTk.PhotoImage(Image.open(os.path.
		join(target,scanList[idx])).crop(roi))
